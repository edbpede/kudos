...
describe("RedisRestRelay", () => {
  ...
  test("expired records are deleted and rejected", async () => {
    ...
    store.set(key, JSON.stringify({ ...record, expiresAt: "2000-01-01T00:00:00.000Z" }));

    expect(relay.readDisplay(created.sessionId, created.displayToken)).rejects.toThrow("expired");
