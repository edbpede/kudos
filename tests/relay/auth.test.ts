...
const totalFor = (display: { students: { id: string; total: number }[] }, studentId: string) =>
...
describe("live relay", () => {
  test("separates teacher and display capabilities", async () => {
    ...
    const display = await relay.applyEvent(created.sessionId, created.teacherToken, {
      studentId,
      delta: 1,
    });
    ...
    const teacherRead = await relay.readTeacher(created.sessionId, created.teacherToken);
    ...
    await expect(relay.readTeacher(created.sessionId, created.displayToken)).rejects.toThrow(
  ...
  test("purge removes remote roster state", async () => {
    ...
    await expect(relay.readDisplay(created.sessionId, created.displayToken)).rejects.toThrow(
      "no longer available",
    );
  });
  ...
  test("expiry rejects reads", async () => {
    ...
    const records = (relay as unknown as { records: Map<string, { expiresAt: string }> }).records;
