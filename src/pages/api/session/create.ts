...
export const POST: APIRoute = async ({ request, url }) => {
  try {
    ...
    const created = await getRelay().create(session, payload.ttlSeconds ?? getLiveTtlSeconds());
    const displayUrl = new URL(`/session/${created.sessionId}/display`, url.origin);
    displayUrl.searchParams.set("token", created.displayToken);
    const teacherUrl = new URL(`/session/${created.sessionId}/teacher`, url.origin);
