...
export class RedisRestRelay implements LiveRelay {
  ...
  private async saveAndDisplay(record: LiveSessionRecord) {
    ...
    const remainingTtl = Math.max(1, Math.ceil((Date.parse(record.expiresAt) - Date.now()) / 1000));
    await this.command([
      "SET",
      this.key(record.id),
      JSON.stringify(record),
  ...
  private async readRecord(sessionId: string): Promise<LiveSessionRecord> {
    const value = await this.command<string | null>(["GET", this.key(sessionId)]);
    if (!value) throw new RelayError("This live session is no longer available.", "NOT_FOUND");
