...
export const deriveTotals = (
  ...
  const totals = Object.fromEntries(session.students.map((student) => [student.id, 0]));
...
export const applyStarEvent = (
  ...
  const student = session.students.find((candidate) => candidate.id === input.studentId);
  if (!student) {
...
export const deriveDisplayState = (session: ClassroomSession, expiresAt?: string): DisplayState => {
  const totals = deriveTotals(session);
  const students: StudentDisplayState[] = alphabetizeStudents(session.students).map((student) => {
    const lastPositiveAt = [...session.events]
      .reverse()
      .find((event) => event.studentId === student.id && event.delta > 0)?.createdAt;
    return {
      id: student.id,
