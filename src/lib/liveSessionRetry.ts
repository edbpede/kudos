...
const TERMINAL_LIVE_ERROR_CODES = new Set(["EXPIRED", "NOT_FOUND", "PURGED", "UNAUTHORIZED"]);
...
export const nextLivePollDelayMs = (pollIntervalMs: number | null | undefined) => {
...
export const nextLiveRetryDelayMs = (failureCount: number, policy: LiveRetryPolicy = {}) => {
  const attempts = Math.max(1, Math.floor(failureCount));
  const baseDelayMs = Math.max(1, Math.round(policy.baseDelayMs ?? LIVE_RETRY_BASE_DELAY_MS));
